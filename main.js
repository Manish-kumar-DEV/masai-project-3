...
function getCountryDetails(lat, lng) {
  ...
  xhr.open(
    "GET",
    "http://api.geonames.org/countrySubdivisionJSON?lat=" + //since this is a http request it will not serve, over https, hence can't be deployed to an https server.
      lat +
      "&lng=" +
      lng +
      "&username=manish_dev"
...
function getDefaultHolidays(country) {
  ...
  xhr.open(
    "GET",
    "https://calendarific.com/api/v2/holidays?api_key=b51e1c17930ef91acaee43f710b2f599e820390a&country=" +
      country +
      "&year=" +
      presentYear
  );
